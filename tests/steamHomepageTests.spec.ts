...
test('Display Top Sellers section', async ({ page }) => {
    ...
    await expect(homePage.topSellersSection).toHaveAttribute('aria-selected', 'true');
});
...
test('Display Special Offers section', async ({ page }) => {
    ...
    await expect(homePage.specialOffersSection).toHaveAttribute('aria-selected', 'true');
});
...
test('Changing language and assert language is really changed', async ({ page }) => {
    ...
    // this wait is not mandatory but added to visually see the submenu opening during test run
